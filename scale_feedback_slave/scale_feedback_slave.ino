/*
README — Scale Slave Firmware (Arduino Nano Every + HX711 + 3-digit 7-seg + I2C)

What this does
- Modes:
  • IDLE: display blank, Green LED OFF.
  • READING: display live weight (0…999 g); <100 → XX.X, ≥100 → XXX; Green LED ON.
  • CAL: display “CAL”; place known weight; short press to compute new scale factor (no tare here).
- Tare (“SEt” shown while sampling 16 points):
  • On startup (auto-tare, shows “SEt”).
  • On physical short press (>50 ms, <3 s).
  • On I2C ‘R’ (master arms read-with-target; this also tares first, shows “SEt”).
  • Long press (≥3 s) enters CAL immediately with a single silent tare at entry (no “SEt” here).
- Target & LEDs:
  • Master writes ‘R’ + target_x10 (LE) → slave tares (shows “SEt”), then arms target.
  • When weight ≥ target → Red LED ON (latched), 1-shot “hit” flag for next I2C read (then clears).
  • Red LED clears on any tare (startup, short press, or I2C ‘R’).
- I2C:
  • Address: 0x2A (change const I2C_ADDR).
  • Write: ‘R’ + uint16 target_x10 (grams×10).
  • Read: 3 bytes [ hit_once(0/1), weight_x10_lo, weight_x10_hi ].
- Overflow:
  • If actual weight > 999 g, show “HI” on the 7-segment; I2C still reports 999.0 max.
- Auto-IDLE: after 30 s without ≥1 g change while in READING → blank display, Green OFF.
- Persistence: **scale saved to EEPROM only after completing CAL**. Tare is NOT persisted.

Fix included in this build
- Eliminates occasional loss of the last digit in 0.X (tearing) by making every display update
  (all 3 digits + decimal points) atomic. No logic changed.

Calibration rule (final)
- Enter CAL with long press (>3 s): immediately show “CAL” and perform ONE silent 16-sample tare at entry only.
- While in CAL, when you press the button again (short press), DO NOT tare; instead:
  • Average samples (32), compute new scale: scale = (raw_avg − tareOffset) / CAL_WEIGHT_G
  • Save scale to EEPROM and return to READING with the new scale factor.
- We only tare outside CAL and once when entering CAL.
*/
...
/********** I2C **********/
const uint8_t I2C_ADDR = 0x2B;  // slave address

/********** TIMING **********/
const uint16_t SCAN_INTERVAL_US = 2000;    // per-digit scan (smaller=brighter)
const uint32_t BTN_DEBOUNCE_MS  = 20;
const uint32_t BTN_LONG_MS      = 3000;
const uint32_t INACTIVITY_MS    = 60000;   // 30 s to drop to IDLE
...
float CAL_WEIGHT_G = 97.6f;      // known weight for calibration (grams)
...
volatile uint16_t g_weight_x10 = 0;       // cached weight in grams*10 for fast I2C onRequest
...
// Average N samples *without* freezing display (used in tare/calib)
...
void display_scanStep() {
  ...
  g_scanIdx = (g_scanIdx + 1) % 3;
  uint8_t glyph = g_glyph[g_scanIdx]; bool dp = g_dp[g_scanIdx];
...
// (Not used directly in loop below, but kept with atomic writes if you call it)
...
void armTarget(uint16_t target_x10) {
  ...
  noInterrupts(); g_targetX10 = target_x10; g_targetActive = true; interrupts();
...
void queueI2CArm(uint16_t tx10) {
  noInterrupts(); g_i2cPendingTargetX10 = (tx10 > 9990) ? 9990 : tx10; g_i2cCmdPending = true; interrupts();
}
...
void loop() {
  ...
  if (g_mode == MODE_CAL_WAIT_WEIGHT) {
    ...
    // Serial diagnostics ~5 Hz (optional)
    uint32_t now = millis();
